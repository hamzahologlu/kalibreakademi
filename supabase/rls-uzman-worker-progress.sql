-- Uzman/Admin: atadığı şirketlerdeki personeli ve ilgili quiz_results satırlarını görsün.
-- Supabase SQL Editor'da bir kez çalıştırın (rls-registration.sql ve rls-sinav-worker.sql sonrası).
--
-- NOT: Admin kontrolü için profiles'a doğrudan alt sorgu KULLANILMAZ — RLS özyinelemesi
-- girişte "Profil bilgisi alınamadı" hatasına yol açar. auth_is_admin() SECURITY DEFINER'dır.
